...
export default function QuestionOfTheDay() {
  const questions = [
    ...
    {
      question:
        'What is something your partner did recently that you are grateful for?',
    },
    {
    ...
    {
      question:
        'What is the worst gift you have ever received, and what did you do about it?',
    },
    {
      question:
    ...
    {
      question:
        'What is something that everyone else seems to love - that you do not understand?',
    },
    {
  ...
  useEffect(() => {
    const questionExists = localStorage.getItem('questionOfTheDay');
    let time = localStorage.getItem('questionOfTheDayDate');
    ...
    if (questionExists && time) {
      // set time to date format
      time = new Date(time);
      if (currentTime === time.getDate()) {
        setQuestionOfTheDay(questionExists);
        return;
      }
    }
    localStorage.setItem('questionOfTheDay', randomQuestion);
    localStorage.setItem('questionOfTheDayDate', new Date().toLocaleDateString());
